...
//codigo do stor
    ...
    sscanf(buf,"227 Passive Mode (%i,%i,%i,%i,%i,%i).",&h1,&h2,&h3,&h4,&p1,&p2);
    pasvport= p1*256+p2;
    ...
    server_addr2.sin_addr.s_addr = inet_addr(buf);
    server_addr2.sin_port = htons(pasvport);
    ...
    bytes=write(sockfd,buf2,strlen(buf2));
    if (bytes > 0)
        printf("Bytes que foram escritos %ld\n", bytes);
    else {
        perror("write()");
        exit(-1);
    }
